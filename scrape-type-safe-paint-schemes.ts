...
async function scrapeDynamicPaintSchemes(options: ScrapeOptions = {}): Promise<void> {
  const {
    years = [2025, 2024, 2023],
    teams = ['hendrick-motorsports', 'joe-gibbs-racing', 'team-penske', 'stewart-haas-racing'],
    carNumbers = ['1', '2', '3', '4', '5', '6', '8', '9', '11', '12', '14', '17', '18', '19', '20', '22', '23', '24', '45', '48'],
    maxSchemes = 100
  } = options;
...
function extractSchemes(html: string, carNumber: string, year: number): PaintScheme[] {
  ...
  $('img').each((_, img) => {
    ...
    if (src.includes('paint') || src.includes('scheme') || alt.includes(carNumber)) {
...
async function saveSchemes(schemes: PaintScheme[], year: number): Promise<void> {
  const client = await pool.connect();

  try {
    for (const scheme of schemes) {
      const driverId = scheme.driverName ? 
        scheme.driverName.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '') :
      ...
      await client.query(`
        ...
        DO UPDATE SET 
          ...
          scheme_image_url = $6,
          year = $7
      `, [`season_${year}`, driverId, scheme.carNumber, scheme.primarySponsor, scheme.schemeType, scheme.imageUrl, year]);
    }
  } finally {
    client.release();
  }
